...
- **(c) Avatar-source shots** — *only if* training/using a new HeyGen look. Stricter bar (locked tripod, even light, held framing, 2–3 min of clean single-voice source per the verified HeyGen Digital Twin requirement). If the chosen look is already trained, skip this and say so — don't invent avatar shots.

**Agent packet (Graeham)** — the **full shooting script**, word for word. Every line he speaks, in delivery order, each with a stage direction (where he is, what he does, what he gestures at) and a label: `[LIVE TO CAMERA]`, `[AVATAR SOURCE — look]`, or `[VOICEOVER — record separately]`. Plus the exact CTA wording and a talking-points block. Not a summary — the actual words. If only an existing avatar delivers the video, the script is still here (it's what the avatar says), but be honest about whether any live filming of