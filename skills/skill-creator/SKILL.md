...
> 3. If the skill is invoked by automation (n8n cadences, Claude Code scheduled tasks, PCFS), confirm the automation reads the canonical SKILL.md from `Documents\Claude\Skills\skills\<skill>\` at run time (scheduled-task wrappers in `Documents\Claude\Scheduled\` must point there). Cached skill content is never the source of truth.
