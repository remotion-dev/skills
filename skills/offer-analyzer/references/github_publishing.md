...
```
<see notes — token stored in cma-generator/references/github_publishing.md or in the local credential file at outputs/.claude-credentials/github-pat.txt>
```
...
fetch('https://api.github.com/repos/Graehamwatts/cma-reports/contents/' + FILENAME, {
  ...
  const body = {
    message: 'Add offer comparison for [full street address]',
    content: window._b64
  };
